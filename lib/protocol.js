...
Protocol.strencode = function (str) {
    ...
    var _buffer = byteArray.slice(0, offset);
    return _buffer;
};
...
Protocol.strdecode = function (buffer) {
    ...
    //console.log('bytes,buffer',typeof buffer, bytes, buffer);
    var array = [];
    var offset = 0;
    var charCode = 0;
...
Package.encode = function (type, body) {
    var buffer = new Buffer(PKG_HEAD_BYTES);
...
Package.decode = function (bytes) {
    ...
    while (offset < bytes.length) {
        ...
        var body = length ? new Buffer() : null;
        if (body) {
            body.write(bytes.slice(offset, offset + length));
        }
        offset += length;
...
Message.decode = function (bytes) {
    ...
    if (msgHasRoute(type)) {
        if (compressRoute) {
            ...
            if (routeLen) {
                route = bytes.slice(offset, offset + routeLen);
    ...
    var body = bytes.slice(offset, offset + bodyLen);
